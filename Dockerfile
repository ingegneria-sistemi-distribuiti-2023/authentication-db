...
# base build command: docker build -t authentication-db:0.01 -f Dockerfile --build-arg EXPOSED_PORT=3306 --build-arg MYSQL_ROOT_PASSWORD=root .
# run command: docker run -d -p 3306:3306 --name authentication-db -e MYSQL_ROOT_PASSWORD=root authentication-db:0.01
